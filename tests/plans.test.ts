...
import { CLASS_INFO, planScopeOf, planKey } from "../src/lib/plans";
...
describe("planKey", () => {
  test("a plan is identified by processor and group together", () => {
    assert.equal(planKey("610455", "KS2336"), "610455|KS2336");
    assert.notEqual(planKey("610455", "KS2336"), planKey("610014", "KS2336"));
  });
  test("a missing group is its own key, not a collision with every other blank", () => {
    assert.equal(planKey("028249", null), "028249|");
    assert.notEqual(planKey("028249", null), planKey("610014", null));
  });
});
