...
async function loadProductsExtras() {
  ...
  for (const r of nadac) {
    ...
    groupByNdc.set(
      r.ndc11,
      groupKey({ ndc11: r.ndc11, equivalenceKey: directory.get(r.ndc11)?.key ?? null, teCode: directory.get(r.ndc11)?.teCode ?? null, description: r.description, classification: r.classification ?? directory.get(r.ndc11)?.classification ?? null, pricingUnit: r.pricingUnit, otc: r.otc ?? directory.get(r.ndc11)?.otc ?? false }),
    );
  }
