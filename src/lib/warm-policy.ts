...
/**
 * Nobody for ninety seconds is a gap; nobody for two minutes is a lull.
 *
 * Five seconds when this was written, and the module's own note says why that cannot hold: a step is not interruptible,
 * and the first-tier steps are the heavy ones — every fill, the product ledger, the books. On 15 September the owner
 * could not sign in. Every press succeeded (thirteen `login.success` rows in two minutes, the password right each time)
 * and he pressed about every thirteen seconds; each five-second gap let the next heavy step start, and libsql holds the
 * event loop for the whole of it, so the page after the redirect never arrived before he pressed again. The sign-in page
 * measured 0.2s on one request and 12s on the next.
 *
 * Ninety is the figure `whenIdle` already waits for before it starts the warm-up at all, so the guard between steps now
 * asks the same question as the guard that let it begin rather than a looser one.
 */
export const GAP_SECONDS = 90;
