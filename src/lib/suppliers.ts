...
const COLUMNS = {
  ...
  itemNumber: ["item number", "item #", "item no", "item no.", "item id", "item code", "supplier item number", "supplier item", "sku", "product code", "catalog number", "catalogue number", "mck item", "vendor item"],
...
async function writeSection(
  ...
  for (const [ndc11, group] of byNdc) {
    ...
    rows2.push({
      ...
      supplierId,
      ndc11,
      itemNumber: pick.itemNumber?.trim() || null,
...
export async function importSupplierCatalog(
  ...
  for (const r of rows) {
    ...
    rows2.push({
      ...
      supplierId,
      ndc11,
      itemNumber: (g("itemNumber") ?? "").trim() || null,
      description,
      productKey: key,
