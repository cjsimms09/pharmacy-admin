...
function inflate(chunk: Buffer): string | null {
  try {
    ...
    try {
      ...
      /*
       * A stream wrapped in ASCII85 before it was compressed: `/Filter [/ASCII85Decode /FlateDecode]`.
       *
       * Heartland's merchant statement, generated by OpenText Exstream, writes every page that way,
       * and this reader got nothing at all out of it — the Flate attempt failed on the ASCII85 text,
       * the raw fallback carried no `Td` or `Tm`, and every page was skipped as not being text. The
       * statement is the pharmacy's only record of what card processing costs.
       *
       * Recognised by its bytes rather than by the object's dictionary, because this reads streams
       * in file order without resolving their owners — the same way everything else here works.
       */
      const a85 = ascii85(chunk);
      if (a85 === null) return null;
      try {
        return zlib.inflateSync(a85).toString("latin1");
      } catch {
        return a85.toString("latin1");
      }
    }
  }
}
...
/**
 * ASCII85 to bytes, or null where the chunk is not ASCII85.
 *
 * Only a chunk made entirely of the ASCII85 alphabet and ending in its `~>` marker is treated as one,
 * so a binary stream that failed to inflate for some other reason is left as it was rather than
 * decoded into noise.
 */
export function ascii85(chunk: Buffer): Buffer | null {
  let s = chunk.toString("latin1").replace(/\s+/g, "");
  if (s.startsWith("<~")) s = s.slice(2);
  const end = s.indexOf("~>");
  if (end < 0) return null;
  s = s.slice(0, end);
  if (s.length === 0 || !/^[!-uz]+$/.test(s)) return null;
  const out: number[] = [];
  let group: number[] = [];
  for (const ch of s) {
