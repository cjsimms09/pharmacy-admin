...
export async function fetchNadac(onProgress?: (text: string) => void | Promise<void>): Promise<FetchResult> {
   ...
   *
   * Which addresses exist is discovered from the CMS listing rather than hard-coded, so the yearly
   * renumbering each January does not quietly break the fetch. Which of them are *worth asking
   * for* is decided here: each weekly file carries the Wednesday in its own address, so a week
   * already in the database has nothing to teach and is never downloaded again. Together they make
   * the daily check both current and free — on an ordinary day it finds every week held and stops
   * without a byte transferred.
   */
  const { heldWeeks } = await import("./nadac");
  const held = await heldWeeks();
  const discovered = await weeklySourcesAsync(s.nadac_source_url);
  const sources = discovered.filter((u) => {
    const week = weekOfUrl(u);
    return week === null || !held.has(week);
  });
  if (sources.length === 0) {
    // Every week this knows how to ask for is already held. Saying so plainly is the point: it is
    // the ordinary outcome of a daily check, and it costs no download at all.
