import { SIGNABLE, contentChanged, type RecordSignature } from "@/lib/record-signatures";
    ...
    return (
      <div className="print-signature mt-6 border border-black p-3 text-[10px] leading-relaxed">
        ...
        <dl className="mt-2 grid grid-cols-2 gap-x-6 gap-y-0.5">
          ...
          <div>
            <span className="font-semibold">Signed</span> {new Date(signature.signedAt).toLocaleString()}
          </div>
          <div>
            <span className="font-semibold">From</span> {signature.signedIp ?? "address not recorded"}
          </div>
        </dl>
        <p className="mt-2 text-[9px]">
          Signed electronically under the Electronic Signatures in Global and National Commerce Act (15 U.S.C. 7001)
          and the Kansas Uniform Electronic Transactions Act (K.S.A. 16-1601 et seq.). The signature, the exact wording
          agreed to, the time, and the identity of the signer are retained in this pharmacy&rsquo;s compliance system
          and are reproducible on request.
        </p>
        ...
        */}
        {changed && (
          <p className="mt-2 border border-black p-1.5 text-[9px] font-semibold">
            This record has changed since it was signed. The signature above stands against the version as it was on
            {" "}
            {new Date(signature.signedAt).toLocaleDateString()}. Withdraw it and sign again if the current figures are
            the ones being certified.
          </p>
        )}
      </div>
    );
  ...
  return (
    <div className="print-signature mt-6">
      ...
      <div className="hidden print:block text-xs">
        <p>{spec.statement}</p>
        <p className="mt-6">
          {defaultName}, pharmacist-in-charge: ______________________________ Date: ______________
        </p>
      </div>

      {canSign ? (
        <div className="no-print rounded-lg border border-line bg-ground p-4">
          <h3 className="text-sm font-semibold">Sign this record</h3>
          ...
          <form action={action} className="mt-3 space-y-3">
            ...
            {content !== undefined && <input type="hidden" name="content" value={content} />}
            ...
            */}
            <label className="flex items-start gap-2 text-sm">
              ...
              <span>I am signing this record, and I agree to the statement above.</span>
            </label>
            ...
            <button className="btn btn-primary">Sign it</button>
          </form>
          <p className="mt-2 text-xs text-ink-3">
            Your name, the time, and the address you are signing from are recorded with the statement. That is what
            makes this the equal of ink under the ESIGN Act and the Kansas UETA — and it is more than a signature on
            paper carries. Print it afterwards and the signature prints with it.
          </p>
        </div>
      ) : (
