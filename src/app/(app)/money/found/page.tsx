...
export default async function MoneyPage({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  async function mark(fd: FormData) {
    ...
    if (!id || (status !== "acted" && status !== "dismissed" && status !== "open")) redirect("/money/found?error=" + encodeURIComponent("Nothing to record."));
    await markRecommendation(id, status, note);
    revalidatePath("/money/found");
    revalidatePath("/money");
    revalidatePath("/");
    redirect("/money/found?ok=" + encodeURIComponent(status === "acted" ? "Recorded as done. The claims from here on will say what it was worth." : status === "dismissed" ? "Recorded. It stays on the list, marked as not being done, so the total is honest." : "Reopened."));
  }
