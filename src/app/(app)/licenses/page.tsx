...
import { PageHeader, Notice, StatusBadge, Field, Figure, Card, Empty } from "@/components/ui";
...
export default async function LicensesPage({ searchParams }: { searchParams: Promise<{ error?: string; saved?: string }> }) {
  ...
  const dated = creds.filter((c) => c.expiresOn);
  ...
  const noDate = creds.filter((c) => !c.expiresOn);
  const noDoc = creds.filter((c) => credDocs(c.id).length === 0);
  ...
  return (
    <>
      ...
      <Card title="Everything with an expiry date" count={creds.length} subtitle="Pharmacy registration, DEA, CSOS, KMAP, professional liability and property insurance, business licence, PSAO and wholesaler agreements.">
        {creds.length === 0 ? (
