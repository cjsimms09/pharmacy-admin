...
export default async function InspectionPage() {
  ...
  const blocking = r.checks.filter((c) => c.state === "blocking");
  const gaps = r.checks.filter((c) => c.state === "gap");
  const ready = r.checks.filter((c) => c.state === "ready");
    ...
    <>
      ...
      <Card tone={r.blocking > 0 ? "crit" : r.gaps > 0 ? "warn" : "ok"} className="mb-6">
        ...
        <p className="mt-1 text-sm text-ink-2">
          {r.ready} of {r.checks.length} questions can be answered straight from this site today.
        </p>
      </Card>

      <div className="mb-6 grid gap-3 sm:grid-cols-3">
        <Figure value={r.blocking} label="Would be a finding" sub="Fix these first" tone={r.blocking === 0 ? "ok" : "crit"} href="#blocking" />
        <Figure value={r.gaps} label="Weaker than it should be" sub="Answerable, but not well" tone={r.gaps === 0 ? "ok" : "warn"} href="#gaps" />
        <Figure value={r.ready} label="Ready to hand over" sub="Producible on the spot" tone="ok" href="#ready" />
      </div>
          ...
          <li key={c.key} className="py-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-ink-3">{c.authority}</p>
              </div>
