...
export default async function ImportReviewPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const matched = (name: string | null) => {
    ...
    return people.some((p) => n.includes(p.lastName.toLowerCase()) || `${p.firstName} ${p.lastName}`.toLowerCase() === n);
  };

  return (
    <>
      <BackLink href="/cqi/import">Imports</BackLink>
      <PageHeader title="Check what Claude read" subtitle="Correct anything that's wrong, untick anything that shouldn't be saved, then create the records. Nothing is saved until you click the button at the bottom." />
        ...
        <section className="space-y-4">
          ...
          {data.incidents.map((inc, i) => (
            <div key={i} className="card grid gap-3 sm:grid-cols-3">
              ...
              <Field label="Reviewer" hint={matched(inc.reviewerName) ? "matches a staff record" : inc.reviewerName ? "no matching staff record — add the person under Staff first, or leave blank" : undefined}><input name={`inc_${i}_reviewerName`} className="field" defaultValue={inc.reviewerName ?? ""} /></Field>
              ...
              <div className="sm:col-span-3">
                ...
                {inc.employees.length === 0 && <p className="text-xs text-ink-3">None listed.</p>}
                {inc.employees.map((e, k) => (
                  <div key={k} className="mb-1 grid gap-2 sm:grid-cols-4">
                    <input name={`inc_${i}_emp_${k}_name`} className="field" defaultValue={e.name} title="Employee name" />
                    <input className="field" defaultValue={e.licenseNumber ?? ""} readOnly title="License # on form (the staff record's number is what prints)" />
                    <input name={`inc_${i}_emp_${k}_reviewedOn`} type="date" className="field" defaultValue={e.reviewedOn ?? ""} />
                    <input name={`inc_${i}_emp_${k}_reviewedBy`} className="field" defaultValue={e.reviewedBy ?? ""} title="Reviewed by" />
                    {!matched(e.name) && <p className="text-xs text-warn sm:col-span-4">“{e.name}” doesn't match a staff record; it will be noted on the incident instead.</p>}
                  </div>
                ))}
              </div>
