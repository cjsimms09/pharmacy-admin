...
async function main() {
  ...
  /*
   * Sweep the claims into the register first, unless told not to.
   *
   * The register was 211 rows and every one of them had a null PCN — they were all created before
   * the PCN was kept — while the claims carry 279 distinct BIN, PCN and group triples. Only 24 of
   * the register's rows matched a claim at all. Everything downstream of that is inert: a plan the
   * register does not hold cannot be offered a class, and the evidence that would settle it is
   * keyed on the PCN the register does not have.
   *
   * `syncPlanGroups` adds a row per triple and deliberately classifies none of them, so this is
   * additive and decides nothing. --no-sync skips it.
   */
  if (!process.argv.includes("--no-sync")) {
    const { syncPlanGroups } = await import("../src/lib/plans");
    const s = await syncPlanGroups();
    console.log(`register swept: ${s.added} plans added, ${s.total} in the claims.`);
  }
