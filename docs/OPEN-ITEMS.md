...
| ~~**The brand book loses money**~~ | — | **Closed 15 September: a measurement fault.** Two-payer fills counted twice and one reversed fill. Rexulti makes money on every paid fill; Zepbound loses about $30 on two ESI fills. See the entry below. |
...
### ~~The brand book loses money~~ — a measurement fault, closed 15 September

He asked about it ("2"). Six fills carried 118% of the −$2,019.24; the other 176 brand scripts made
+$362.96, and the six ran at 0.556 and 0.495 of cost, an order of magnitude outside the range a
routine brand claim is paid in. Measured by session 1 through `ingredientReceivedCents()`:

- **Two of the six were one fill each counted as two.** Rexulti 335651 and Zepbound 337199 were billed
  to a primary and a savings-card secondary, and a per-row count charged the carton twice. Zepbound
  337199 read 0.545 on remit alone and is 1.021 correctly.
- **One was never sold.** Rexulti 337216 was reversed.
- **Cost was right.** $1,440 a Rexulti fill and $1,046 a Zepbound carton, consistent across rows; a
  Zepbound fill is a full four-pen carton. No unit fault.

**Real result.** Rexulti makes money on every paid fill, at 1.033–1.040. Zepbound loses about $30 on
each of two Express Scripts fills paid 97% of cost and makes $22 on the third. **Nothing near a
do-not-dispense decision**, and the brand book is not losing money on this evidence. The answer given
to "our accrual is too low for the month" rested on this figure and no longer stands on it.

**It reaches further, and is session 1's to rebuild.** September below cost counted per claim row is
$25,522.19, of which $11,951.77 sits on the 76 fills with two payer rows — 2.6% of fills carrying 47%
of the money. Nothing has been filed or shown from those totals; the remedy router is being rebuilt on
fills first. The same row-counting doubles the saving on `/purchasing/products`
(`suppliers.ts loadPurchasingOpportunities`), reported to session 1 15 September. `payer-map.ts` and
`profit-and-loss.ts` were checked and sum over fills, not rows.
