...
| **Credit card batch report** | email, one per business day, forwarded by staff (staff@example.com) | one settlement batch: net card takings, count, card types. No fees. Read by `card-batch.ts`, banked as a `patient` cash receipt keyed `card-batch|<id>`. **Cash only** — accrual already has the revenue from claims and the till. Routed `card_batch`. **Check one arrived for each business day.** |
