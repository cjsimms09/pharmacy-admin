...
| `copay-remit-redsail.txt` | text layer of page 2 of RedSail's "Remittance Advice - RAS Copay Voucher Reimbursement" (page 1 is a scan) | **committed** — prescription numbers, check/ACH number, NPI, pharmacy name and address invented; NDCs, drugs, quantities and every amount kept, because the arithmetic is the point and the rows net to the printed total to the cent. Carries a fill and its reversal, a fill reversed and re-paid at a different submitted amount, two prescriptions alike in every field but the number, and two rows whose prescription number and NDC the text layer breaks across runs; `tests/copay-remit.test.ts` reads it |
